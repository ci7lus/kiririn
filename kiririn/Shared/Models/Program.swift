...
nonisolated struct ProgramGenre: Codable, Sendable, Equatable, Hashable {
    var lv1: Int
    var lv2: Int?
    ...
    var majorDisplayName: String {
        Self.majorDisplayNames[lv1] ?? "不明"
    }

    var subDisplayName: String? {
        guard let lv2,
            let name = Self.subDisplayNames[lv1]?[lv2],
            !name.isEmpty
        else {
            return nil
        }
    ...
    var displayName: String {
        guard let subDisplayName else { return majorDisplayName }
        return "\(majorDisplayName) / \(subDisplayName)"
    }

    static func level1(for displayName: String) -> Int? {
        Self.majorDisplayNames.first { $0.value == displayName }?.key
    }

    static func level2(for displayName: String, in level1: Int) -> Int? {
        guard !displayName.isEmpty else { return nil }
        return Self.subDisplayNames[level1]?.first { $0.value == displayName && !$0.value.isEmpty }?
            .key
    }
}

extension ProgramGenre {
    fileprivate nonisolated static let majorDisplayNames: [Int: String] = [
        0x0: "ニュース・報道",
        0x1: "スポーツ",
        0x2: "情報・ワイドショー",
        0x3: "ドラマ",
    ...
    fileprivate nonisolated static let subDisplayNames: [Int: [Int: String]] = [
        0x0: [
            0x0: "定時・総合",
            0x1: "天気",
            0x2: "特集・ドキュメント",
