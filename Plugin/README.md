...
kiririn のプラグインシステムは WebExtension ベースです。<br>
アプリ固有の機能は `window.kiririn` で公開し、その他の仕様は WebKit の WebExtension API の対応に従います。

## 配布形式

- 配布用パッケージの拡張子は `.kppx` です。
- `.kppx` は有効な ZIP archive 形式ですが、配布用に署名されたものには Android APK Signature Scheme v2 ~ v3.1 と同様に Central Directory の前領域に署名領域が存在します。
...
- `browser_specific_settings.kiririn`: kiririn 関連設定値。必須。
...
- `browser_specific_settings.kiririn.update_url`: アップデート検出用ファイルの URL。定義は [Update manifest](#update-manifest) 参照。
...
`strict_min_version` / `strict_max_version` の判定はアプリの `CFBundleShortVersionString` と数値比較で行います。<br>
通常モードでは互換性を満たさないプラグインは追加・更新できません。<br>
...
`window.kiririn.getRuntimeInfo().displayAreaType` の値は `overlay` / `panel` / `options` です。<br>
...
型定義は [KiririnPluginBridge.d.ts](./KiririnPluginBridge.d.ts) にあります。
...
- `applications.kiririn`: 任意。省略時は互換とみなします。
  - `applications.kiririn.strict_min_version` / `strict_max_version`: アプリの `CFBundleShortVersionString` と照合し、互換性がある更新候補だけを対象にします。
